...
| 属性 | 值 |
|------|-----|
| **URL** | `/api/sns/web/v1/you/mentions` |
| **Method** | `GET` |
| **需要登录** | ✅ 是 |
...
|--------|------|------|------|--------|
| `num` | int | 否 | 每页数量 | `20` |
| `cursor` | string | 否 | 分页游标 | `6952xxxxx` |
...
| 属性 | 值 |
|------|-----|
| **URL** | `/api/sns/web/v1/you/connections` |
| **Method** | `GET` |
| **需要登录** | ✅ 是 |
...
| `cursor` | string | 否 | 分页游标 |
| `top_comment_id` | string | 否 | 置顶评论ID |
| `image_formats` | string | 否 | 图片格式，默认 `jpg,webp,avif` |
| `xsec_token` | string | 是 | 安全令牌，需从Feed流或页面中获取 |

**完整请求示例**

```bash
curl 'https://edith.xiaohongshu.com/api/sns/web/v2/comment/page?note_id=67812xxx&cursor=&top_comment_id=&image_formats=jpg,webp,avif&xsec_token=ABxxx' \
  -H 'x-s: [动态签名]' \
  -H 'x-s-common: [动态签名]' \
  -b '...'
```
